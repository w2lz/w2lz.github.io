...
我们的评估标准时，索引的复用能力。因为可以支持最左前缀，所以当应有了(a, b)这个联合索引后，一般不需要单独在a上建立索引。因此，第一原则是，如果通过调整顺序，可以少维护一个索引，那么这个顺序往往就是需要优先考虑采用的。
...
然后就要判断其他条件是否满足了，在MySQL5.6之前，只能从ID3开始一个一个的回表。到主键索引上找出数据行，在对比字段值。
